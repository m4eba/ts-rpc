...
import { SourceBuilder, generateEventMessageInterface } from '../src/builder';
...
test('message 1 param', async() => {
  ...
  generateEventMessageInterface(builder,i,'hello');
  const genFile = project.createSourceFile('gen.ts',builder.getSource());
